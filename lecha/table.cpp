...
//        std::cout << "size = " << _size << '\n';
        if (count == 1)
        {
            ...
            {
                delete ptr[i];
                std::cout << "delete" << ptr[i]->type << std::endl;
/*              if (ptr[i]->type == B)
                {
                    BoolItem* pointer = dynamic_cast<BoolItem*> (ptr[i]);
                    delete pointer;
                }
                else if (ptr[i]->type == ST)
                {
                    TextItem* pointer = dynamic_cast<TextItem*> (ptr[i]);
                    delete pointer;
                }
                else if (ptr[i]->type == N || ptr[i]->type == I || ptr[i]->type == T)
                {
                    NumItem* pointer = dynamic_cast<NumItem*> (ptr[i]);
                    delete pointer;
                }
                else
                {
